...
function dfs(oldNode, newNode, patches) {
  ...
  diffChildren(oldNode.el, oldNode.childNodes, newNode.childNodes, patches)
...
function diffChildren(parentEl, oldCh, newCh, patches) {
  ...
  while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
    if (oldStartNode === undefined) {
      ...
      console.log(oldStartNode.childNodes, newStartNode.childNodes)
      ...
      moveElement(parentEl, oldStartNode, oldEndIndex + 1 >= oldCh.length ? null : oldCh[oldEndIndex + 1]);
      oldStartIndex += 1;
      newEndIndex -= 1;
      oldStartNode = oldCh[oldStartIndex];
      newEndNode = newCh[newEndIndex];
  ...
  if (oldStartIndex <= oldEndIndex) {
    ...
    addNodes(parentEl, newCh, newStartIndex, newEndIndex, patches);
  }
}

function addNodes(parentEl, ch, start, end, patches) {
  for (let i = start; i <= end; i++) {
    addElement(parentEl, ch[i], null, patches)
  }
}
